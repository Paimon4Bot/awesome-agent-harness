...
As always, it's up to you to decide what's right for your application. But don't think you _have_ to manage them separately.
...
You might get better results by having the LLM _always_ output json, and then declare it's intent with some natural language tokens like `request_human_input` or `done_for_now` (as opposed to a "proper" tool like `check_weather_in_city`).
